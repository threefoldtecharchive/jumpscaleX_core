...
class JSXObjectRoot(JSXObject):
    def _init_pre(self, capnpdata=None, datadict={}, schema=None, model=None, autosave=None):
        ...
        # find the root obj of this one (the tob of the obj)
        if self._parent:
            current = self
    ...
    @property
    def _autosave(self):
        if not self._root._model:
            return False
        if self._root._autosave_ == None:
            return self._root._model.autosave
        return self._root._autosave_
    ...
    def _save_root(self, serialize=True):
        if self._changed:
            ...
            if self._model:
                ...
                # VERY BADLY DONE AND INCREDIBLY SLOW, CANNOT DO LIKE THIS, THE INDEX ON SQLITE NEEDS TO PROTECT US HERE
    ...
    def delete(self):
        if self._parent:
            raise j.exceptions.Base("can only delete root obj, not subobj")
