...
def test_sign_encrypt_decrypt():
    ...
    res = j.me.data_send_serialize(None, data_list, serialization_format="json")
    ...
    sign_data_raw = j.me._serialize(data_list, serialization_format="json").encode()
    ...
    # HOW TO DO THIS?
    def encrypt_result_for_client(res, pubkey_hex):
        assert isinstance(res, list)
        assert len(res) == 3
        data = res[1]
        assert len(pubkey_hex) == 64
        pubkey = PublicKey(binascii.unhexlify(pubkey_hex))
        data3 = j.me.encryptor.encrypt(data, public_key=pubkey)
        res[1] = data3
        return res
    ...
    # )
    res = j.me.data_send_serialize(None, data_list, serialization_format="msgpack")
    res = encrypt_result_for_client(res, binascii.hexlify(client_sk.public_key.encode()))
    ...
    sign_data_raw = j.me._serialize(data_list, serialization_format="msgpack")
    ...
    decrypted = j.me.encryptor.decrypt(res[1], public_key=client_sk.public_key)

    assert sign_data_raw == decrypted

    print("OK")
    j.shell()
    # TODO fix the below cases
