...
class Me(JSConfigBase, j.baseclasses.testtools):
    ...
    def data_send_serialize(self, threebot, data, serialization_format="json"):
        """
        instead of deprecated j.tools.threebot._serialize_sign_encrypt
    ...
    def data_received_unserialize(self, threebot, data, signature, serialization_format="json"):
        ...
        nacl = self.encryptor    # signature used to be verifykey_hex in j.tools.threebot
        # decrypt data
        ...
        if threebot:
            threebot_client = j.clients.threebot.client_get(threebot)
            threebot_client.verify_from_threebot(data=data_dec_ser, signature=signature)
        else:
            sign_is_ok = nacl.verify(data_dec_ser, data[2], verify_key=binascii.unhexlify(signature))
            if not sign_is_ok:
                raise j.exceptions.Base(
                    "could not verify signature:%s, against pubkey:%s" % (data[2], binascii.unhexlify(signature))
                )
        return data_dec
