...
def test_serialization():
    ...
    res_j = j.tools.threebot._serialize(data_list, serialization_format="json")

    print(res_j)
    p = '[true, 1, [1, 2, "a"], [999, "f47f5adfee782b7059616c42b5c10fab", "{\\"name\\": \\"aname\\", \\"description\\": \\"something\\"}"], "astring", [998, {"a": 2}]]'

    assert res_j == p
    res_mp = j.tools.threebot._serialize(data_list, serialization_format="msgpack")
    print(res_mp)
    ...
    un_res_mp = j.tools.threebot._unserialize(res_mp, serialization_format="msgpack")
    print(un_res_mp)

    assert un_res_mp == data_list
    print("test with json format")
    un_res_j = j.tools.threebot._unserialize(res_j, serialization_format="json")
    print(un_res_j)
    ...
    # CLEAN STATE
    j.tools.threebot._log_info("TEST serialization DONE")
    return "OK"
