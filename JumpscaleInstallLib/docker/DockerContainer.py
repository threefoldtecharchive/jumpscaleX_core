...
class DockerContainer:
    ...
    def start(self, stop=False, delete=False, update=False, ssh=None, mount=None, pull=False, image=None, portmap=True):
        ...
        DIR_CODE = self.myenv.config["DIR_CODE"] if not self.myenv.platform_is_windows else "%s\code" % self.myenv._basedir_get()
        DIR_BASE = self.myenv.config["DIR_BASE"] if not self.myenv.platform_is_windows else self.myenv._basedir_get()
        DIR_IDENTITY = f"{DIR_BASE}/myhost" if not self.myenv.platform_is_windows else "%s\myhost" % self.myenv._basedir_get()
    ...
    def _update(self, update=False, ssh=False):
        ...
        if True or ssh or update or not self.config.done_get("ssh"):
            ...
            if self.myenv.platform_is_windows:
                path = f"{self.myenv._homedir_get()}\.ssh\id_rsa.pub"
                if not self.tools.exists(path):
                    raise self.tools.exceptions.Input(f"Please make sure your ssh-key is existed in {path}")
                with open(path) as ssh_key_file:
                    SSHKEYS = ssh_key_file.read().strip("\n")
            else:
                SSHKEYS = self.tools.execute("ssh-add -L", die=False, showout=False)[1]
            ...
            if not self.myenv.platform_is_windows:
                self.tools.execute(
                    "mkdir -p {0}/.ssh && touch {0}/.ssh/known_hosts".format(self.myenv.config["DIR_HOME"], showout=False)
                )
            ...
            if self.myenv.platform_is_windows:
                ...
                path = f"%s\.ssh\known_hosts" % self.myenv._homedir_get()
                _, content, _ = self.tools.execute(cmd, showout=False)
                self.tools.file_write(path, content)
            else:
                cmd = "ssh-keyscan -H -p %s localhost >> %s/.ssh/known_hosts" % (
                    self.config.sshport,
                    self.myenv.config["DIR_HOME"],
                )
                self.tools.execute(cmd, showout=False)
    ...
    def kosmos(self):
        self.execute(
            f"j.application.interactive=True; j.shell()",
            interactive=True,
