...
class DockerFactory:
    ...
    def containers_running():
        ...
        # Windows needs " not '
        names = self.tools.execute('docker ps --format="{{json .Names}}"', showout=False, replace=False)[1].split("\n")
    ...
    def containers_names():
        # IMPORTANT use single quotes in the outer and double in between
        # Windows needs " not '
        names = self.tools.execute('docker container ls -a --format="{{json .Names}}"', showout=False, replace=False)[
            1
        ].split("\n")
        names = [i.strip("\"'") for i in names if i.strip() != ""]
        return names
